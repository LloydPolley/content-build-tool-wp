...
export default function SaleBanner(options) {
  var ref = this;
  this.html = function(element) {
    // element.innerHTML += "<!--mp_trans_disable_start-->";
    element.innerHTML += options.html;
    // element.innerHTML += "<!--mp_trans_disable_end-->";
  };
  ...
  this.createCTA = function(ctaInfo) {
    ...
    element.innerHTML =
      ...
      ctaInfo[1] +
      '</span><span class="nl-inline">' +
      ctaInfo[2] +
      '</span><span class="fr-inline">' +
      ctaInfo[3] +
      '</span><span class="de-inline">' +
      ctaInfo[4] +
      "</span>";
    return element;
  };
  ...
  this.liveArea = function() {
    if (typeof options.cta === "object" && options.cta.length > 0) {
      ...
      liveArea.innerHTML += "<!--mp_trans_disable_start-->";
      if (options.h1) {
      ...
      liveArea.innerHTML += "<!--mp_trans_disable_end-->";
      return liveArea;
    }
  };
  ...
  this.appendToPage = function() {
    ...
    if (targetElement) {
      ...
      if (options.class) {
        saleBanner.setAttribute(
          "class",
          options.class + " image content-promotion"
        );
      }
      ...
      if (options.links) {
        options.links.forEach(function(element, index) {
          saleBanner.appendChild(
            ref.createLink(element.href, element.title, element.class)
          );
        });
      }
      if (options.html) {
    ...
    if (targetMobElement) {
      ...
      mobSaleBanner.setAttribute(
        "class",
        "sale-banner-mob image content-promotion"
      );
      if (options.class) {
        mobSaleBanner.setAttribute(
          "class",
          options.class + "-mob image content-promotion"
        );
      }
...
//     var serverDate;
//     if (
//       document.getElementById("embargoDate") !== null &&
//       document.getElementById("embargoDate").value.length === 16
//     ) {
...
//     var runScript =
//       todayDate >= startDate && todayDate < endDate ? true : false;
//     return runScript;
//   };
